...
namespace nilnul.uinteger
{
	...
	/// <summary>
	/// regards uint[] as a big uint number.
	/// </summary>
	static public partial class UIntsX
	{
		...
		/// <returns></returns>
		static internal uint[] _Add(this uint[] augend, uint addend, int addendExp)
		{
		...
		unsafe
		static internal void _Increase( uint* augend, int augendLen, uint addend, int addendExponent)
		{
		...
		static internal void _Increase(
			this uint[] augend
			, uint addend
			, uint addendExponent
			)
		...
		static internal void Increase(this uint[] augend, uint addend, uint addendExponent)
		{
		...
		/// <param name="b"></param>
		/// <returns></returns>
		static internal uint[] _Add(this uint[] a, uint[] b)
		{
		...
		/// 
		unsafe
		static internal uint[] _Add(uint* a,int aLen, uint* b,int bLen)
		{
		...
		static internal void Decrease(this uint[] a, uint[] b)
		{
		...
		unsafe
		static internal void _Decrease(uint* a,int aLen, uint* b,int bLen, int bExponent)
		{
		...
		unsafe
		static internal void _Decrease(uint[] a, uint* b, uint bLen)
		{
			for (uint i = 0; i < bLen ; i++)
			{
				_Decrease(a, b[i], i);
		...
		unsafe
		static internal void _Decrease(uint[] a, uint* b, int bLen, uint bExponent)
		{
		...
		unsafe
		static internal void _Decrease(uint* a, int aLen, uint[] b, int bExponent)
		{
		...
		/// <param name="p"></param>
		static internal void _Decrease(
			this uint[] a,
			uint b,		//b>0
			uint bExponent)
		{
		...
		/// <param name="b"></param>
		/// <param name="p"></param>
		static internal void Decrease(this uint[] a, uint b, uint bExponent)
		{
			if (b > 0)
			{
		...
		static public uint[] Quotient(this uint[] dividend,uint[] divisor)
		{
			uint[] dividend2remainder = new uint[dividend.Length];
			dividend2remainder = dividend.ToArray();
			return QuotientRemainder(dividend2remainder, divisor);
		}
		...
		static public int Compare(this uint[] a,uint[] b)
		{
			if (a.Length>=b.Length)
			{
				return _Compare(a,b);
		...
		/// <param name="b"></param>
		/// <returns></returns>
		private static int _Compare(uint[] a, uint[] b)
		{
		...
		static private int _Compare(this uint[] a, int aLow, int aLen, uint b)
		{
		...
		/// <param name="divisorLow"></param>
		/// <param name="divisorLen"></param>
		/// <returns>the quotient. the remainder is discarded.</returns>


		static private uint _Quotient(this uint[] dividendArray, int dividendLow, int dividendLen, uint[] divisorArray,int divisorLow,int divisorLen)
		{
		...
		/// 
		/// </summary>
		/// <param name="dividend2remainder">remainder will be left</param>
		...
		)
		{
			...
			for (int quotientPos = quotient.Length-1; quotientPos >= 0; quotientPos--)
			{
				...
				quotient[quotientPos] =
					dividend2remainder._Quotient(

						quotientPos	
						,
						dividend2remainder.ValidHigh()-quotientPos+1
						,
						divisor
						,
						divisorLow
		...
		/// 
		/// </summary>
		/// <param name="dividend_remainder"></param>
		/// <param name="divisor"></param>
		/// <returns>the quotiend. and the dividend is changed, to be the remainder, which is in [0,divisor)</returns>
		static public uint[] QuotientRemainder(this uint[] dividend_remainder,uint[] divisor) {


			int dividendValidHigh = dividend_remainder.ValidHigh();
			...
			return _QuntientRemainder(dividend_remainder, 0, dividendValidHigh + 1, divisor, 0, divisorValidHigh + 1);
		...
		static public string ToStr(this uint[] a,uint n)
		{
			...
			if (a2.ValidHigh() < 0)
			{
				...
				if (a2._Compare(new uint[] { n }, 0, 1) < 0)
				{
				...
				{
					uint[] r = a2.QuotientRemainder(new uint[] { n });

					return r.ToStr(n) + a2.ToStr(n);
				}

		}

		static public string ToHexString(this uint[] a)
		{
			return a.Aggregate("", (r,c) => r = c.ToString("X8")+ r);
